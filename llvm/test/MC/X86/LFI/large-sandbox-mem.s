...
// Large-sandbox mode masks addresses with the r15 mask register instead of the
// default 32-bit truncation. A plain mov must not clobber the flags, so it
// masks with pext; a read-modify-write already sets them and uses andq.

// Load through a register: the destination doubles as the scratch register.
movq (%rax), %rdi
// CHECK:      .bundle_lock
// CHECK-NEXT: pextq %r15, %rax, %rdi
...
adcq %rax, (%rbx)
// CHECK:      .bundle_lock
// CHECK-NEXT: pextq %r15, %rbx, %r11
// CHECK-NEXT: adcq %rax, (%r14,%r11)
// CHECK-NEXT: .bundle_unlock

sbbq %rcx, (%rdx)
// CHECK:      .bundle_lock
// CHECK-NEXT: pextq %r15, %rdx, %r11
