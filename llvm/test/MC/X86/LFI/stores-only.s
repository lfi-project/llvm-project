...
// TLS with offset (load) - TP rewrite, but in stores-only mode the load
// itself is not sandboxed.
movq %fs:8, %rax
// CHECK:      movq 16(%r15), %rax
// CHECK-NEXT: movq 8(%rax), %rax

// TLS with register base (load) - TP rewrite, no sandbox in stores-only mode.
movq %fs:(%rcx), %rax
// CHECK:      movq 16(%r15), %rax
// CHECK-NEXT: movq (%rax,%rcx), %rax

// TLS with register base and offset (load) - TP rewrite, no sandbox.
movq %fs:16(%rcx), %rax
// CHECK:      movq 16(%r15), %rax
// CHECK-NEXT: movq 16(%rax,%rcx), %rax
