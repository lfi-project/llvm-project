...
//
// Forward-edge control-flow integrity is enforced with ENDBR64 landing pads
// (see X86LFIRewritePass and the emitCFICheck/emitSandboxBranchReg helpers).
// Because every valid indirect-branch target is a 32-byte-aligned ENDBR64
// instruction, the multi-instruction memory-sandbox sequences below cannot be
// entered in the middle by an indirect branch, so -- unlike the bundling-based
// scheme -- they do not need to be wrapped in .bundle_lock/.bundle_unlock.
//
//===----------------------------------------------------------------------===//
...
// Lazily emit the weak ``_lfi_trap`` symbol into a COMDAT ``.text_lfi_trap``
// section so that all translation units share one copy. The symbol contains a
// single ``ud2`` (0f 0b).
MCSymbol *
...
// Emit the alignment, optional forward-edge CFI check, and base-relocation for
// an indirect-branch target held in Reg. In the fixed-4GiB scheme:
//
//   andl $-32, %eX
...
//   addq %r14, %rX
//
// In large-sandbox mode the target is first masked to the sandbox with %r13,
// then aligned with a 64-bit and (which, unlike andl, does not clear the high
// sandbox bits):
//
//   andq %r13, %rX
//   andq $-32, %rX
...
void X86::X86MCLFIRewriter::emitSandboxBranchReg(MCRegister Reg,
  ...
  if (hasLargeSandbox(STI)) {
    ...
    AndAlign.addOperand(MCOperand::createReg(Reg64));
    AndAlign.addOperand(MCOperand::createImm(-IndBranchAlignment));
    ...
    And.addOperand(MCOperand::createReg(Reg32));
    And.addOperand(MCOperand::createImm(-IndBranchAlignment));
...
// Pad before a direct call to a `returns_twice` function so that the byte after
// the call lands on a 32-byte boundary, then emit ENDBR64 there. This is the
// address that setjmp saves into the jmp_buf and that longjmp later reaches via
// an indirect jmp; the alignment + endbr64 lets that indirect jmp pass the
...
// Expand an indirect call or jump (register or memory operand variant) into the
// sandboxed sequence above followed by the original branch instruction.
...
// GS-context mode each TPOffset(%r15) below is instead %gs:TPOffset. The
// resulting non-%fs access is re-dispatched so the regular load/store path
// memory-sandboxes it. Example rewrites (default configuration):
//
// movq %fs:0, %rax
// ->
// movq 16(%r15), %rax
...
bool X86::X86MCLFIRewriter::emitSandboxMemOps(MCInst &Inst,
  ...
  bool EmittedSandbox = false;

  // Iterate only the declared operands: memory operands are always within the
  // fixed operand list, while a variadic instruction (e.g. a return carrying an
  // implicit value operand) may have extra MCInst operands beyond OpInfo.
  for (int I = 0, E = OpInfo.size(); I < E; ++I) {
    if (OpInfo[I].OperandType != MCOI::OPERAND_MEMORY)
      continue;

    prepareSandboxMemOp(Inst, I, ScratchReg, Out, STI);

    if (willEmitSandboxInsts(Inst, I)) {
      // Probe mode: report that sandboxing is needed without emitting anything.
      if (!EmitInstructions)
        return true;
      EmittedSandbox = true;
    }
...
static void demoteInst(MCInst &Inst, const MCInstrInfo &InstInfo) {
  ...
  // Demote any 64-bit GPR operands to their 32-bit counterparts. Iterate only
  // the declared operands so variadic instructions with extra MCInst operands
  // do not index past OpInfo.
...
void X86::X86MCLFIRewriter::rewriteStackModification(MCRegister StackReg,
  ...
  // Sandbox any memory operand of the modifying instruction (e.g. the load in
  // "addq (%rax), %rsp").
...
void X86::X86MCLFIRewriter::doRewriteInst(const MCInst &Inst, MCStreamer &Out,
  ...
  // Forward-edge CFI: rewrite indirect calls and jumps (register or memory
  // operand). Indirect branches are dispatched before isFSAccess so that an
  // indirect call/jmp through an %fs-segment memory operand (e.g. callq *%fs:0)
  // goes through expandIndirectBranch first; the inner load it emits will then
  ...
  // Direct calls to returns_twice functions need their return address aligned
  // and given an ENDBR64 landing pad (for the longjmp path). Other direct
  // calls and jumps have link-time-fixed targets and pass through unchanged.
