...
// Not required for soundness, but it simplifies verification.
...
// Mark each memory access across which EFLAGS is dead with a dead implicit
// EFLAGS def, so the LFI rewriter can mask its address with andq (which
// clobbers EFLAGS) instead of the slower pext.
static bool markDeadFlagsMemOps(MachineBasicBlock &MBB,
  ...
  for (MachineBasicBlock::iterator I = MBB.begin(), E = MBB.end(); I != E; ++I) {
    ...
    if (Desc.hasImplicitDefOfPhysReg(X86::EFLAGS) ||
      ...
      continue;
    // Compute liveness on the unmodified block, then mark.
