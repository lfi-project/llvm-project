...
/// Returns a copy of Inst with the register at operand index Idx replaced by
/// NewReg.
...
// Forward declarations for RoW conversion and pre/post-index helper tables.
...
void AArch64MCLFIRewriter::rewriteLoadStoreBasic(const MCInst &Inst,
  ...
  if (Info->IsPrePost) {
    ...
    if (BaseOpcode != AArch64::INSTRUCTION_LIST_END) {
      ...
      // Update the base register with the offset.
      if (Info->OffsetIdx >= 0) {
...
void AArch64MCLFIRewriter::rewriteStackModification(
  ...
  // If this is a load/store that also modifies SP (e.g. ldp x29, x30, [sp],
  // #16), handle the memory access and guard LR if it was modified.
  ...
  // In jumps-only mode, no stack sandboxing.
  bool SkipLoads = STI.hasFeature(AArch64::FeatureNoLFILoads);
  bool SkipStores = STI.hasFeature(AArch64::FeatureNoLFIStores);
...
static std::optional<MemInstInfo> getMemInstInfo(unsigned Op) {
  ...
  case AArch64::PRFMl:
    ...
    // Scalar indexed/unscaled/register-offset.
#define X(NAME, S)         \
...
#undef X
  ...
  case AArch64::STLLRW:
  case AArch64::STLLRX:
    // SIMD multiple/replicate (base form).
