...
/// Compute the padding size to boundary-align the fragments BF is
/// responsible for.
static uint64_t computeBoundaryAlignSize(const MCAssembler &Asm,
  ...
  if (AlignedSize > Asm.getBundleAlign().value()) {
    Asm.recordError(BF.getLoc(), "fragment can't be larger than a bundle size");
    return 0;
  }
  if (BF.isAlignToEnd())
