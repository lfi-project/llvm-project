...
static bool ExecuteAssemblerImpl(AssemblerInvocation &Opts,
  ...
  if (Opts.OutputType == AssemblerInvocation::FT_Asm) {
    ...
    assert(Opts.OutputType == AssemblerInvocation::FT_Obj &&
           "Invalid file type!");
        if (llvm::Triple(Opts.Triple).isLFI()) {
      ...
      Str.reset(TheTarget->createAsmStreamer(Ctx, std::move(TempOut), std::move(IP),
                                             std::move(CE), std::move(MAB)));
    } else {
  ...
  if (triple.isLFI()) {
    ...
    std::stringstream SS;
    SS << Prog.get() << " " << LFIFlags << " " << "-a " << TheTarget->getName() << " " << AsmTemp->TmpName << " -o " << RewriteTemp->TmpName << "\n";
    if (LFIDebug)
      errs() << SS.str();
